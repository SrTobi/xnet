...
namespace xnet {
	namespace serialization {
		...
		{
			auto* newNode = _document->allocate_node(rapidxml::node_element, ELEMENT_NAME);
			_currentNode->append_node(newNode);

			if (tag)
			{
				auto* attr = _document->allocate_attribute(TAG_NAME, tag);
		...
		{
			{
				auto* attr = _currentNode->first_attribute(TAG_NAME);
				BOOST_ASSERT(!attr == !tag);
				if (tag)
				{
					BOOST_ASSERT(attr->value() == tag);
				}
				BOOST_ASSERT(_currentNode->name() == ELEMENT_NAME);
		...
		{
			BOOST_ASSERT(_currentNode->first_attribute(TYPE_NAME) == nullptr);
			auto* attr = _document->allocate_attribute(TYPE_NAME, type);
		...
		xml_sink::xml_node* xml_sink::_save_string(const std::string& out, const char* type)
		{
			...
			auto* newNode = _document->allocate_node(rapidxml::node_element, PRIMITIVE_NAME, text, 0, out.size());
