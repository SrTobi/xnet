...
namespace xnet {
	...
	public:
		...
		template<typename T>
		package deserialze(T& out, next_package_option nextOpt = request_next_package) const
		{
			package next = _content->deserialize([&out](serialization::generic_source& source)
			{
				serialization::deserializer<serialization::generic_source> deserializer(source);
				deserializer >> out;
			});
	...
	{
		T out;
		nextPkg = pack.deserialze(out, nextOpt);
		return out;
	}
}
